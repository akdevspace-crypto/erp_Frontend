...
export function PatientManualBilling() {
  ...
  const handlePatientSelect = async (id: string, name?: string) => {
    setPatientId(id);
    if (name) setPatientName(name);

    if (!id || billType === "HISTORY") return;

    try {
      const res = await api.get(`/api/v1/operations/patients/${id}`);
      if (res.data?.success && res.data?.data) {
        ...
        if (p.age) {
          ...
          const calculatedAge = Math.floor((new Date().getTime() - new Date(p.dob).getTime()) / 31557600000);
          setPatientAge(String(calculatedAge));
        }
        
        if (p.gender) setPatientSex(p.gender);
        if (p.dob) setPatientDob(p.dob.split('T')[0]);
        
        const gName = p.emergencyContact || p.primaryContact;
        if (gName) setGuardianName(gName);
        
        const gContact = p.phone || p.primaryContact;
        if (gContact) setGuardianContact(gContact);
        
        if (p.address) {
          setGuardianAddress(prev => ({ ...prev, street: p.address }));
        }
      }
    } catch (error) {
    ...
    <div class="sheet">
        <div class="header">
            <div class="brand">
                ...
                <strong style="font-size: 13px; margin-top: 8px; color: #64748b; letter-spacing: 0.05em;">(A Unit Of UNCF)</strong>
            </div>
            <div class="title">
