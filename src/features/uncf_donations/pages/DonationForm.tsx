...
export const DonationForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        // Section 3: References & Tax & Admin
        references: [
            { name: '', mobile: '' },
            { name: '', mobile: '' },
            { name: '', mobile: '' },
        ...
        preferredPrayerDate: '',
        honouredPersonImage: '',
        specialPrayerMessage: '',
        wishToVisitHome: false,
        preferredVisitDate: ''
    });
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setFormData(prev => ({ ...prev, honouredPersonImage: reader.result as string }));
            };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        let finalMaterialDetails = formData.materialDetails;
        if (formData.paymentMode === 'MATERIALS') {
            // Keep material details as is (we removed the Old/New logic as per the new image, but we can keep it if needed, the user just wants the input box)
        } else if (formData.paymentMode === 'UPI') {
            finalMaterialDetails = `UPI TXN: ${formData.transactionId}`;
        } else if (formData.paymentMode === 'CHEQUE') {
            finalMaterialDetails = `Cheque No: ${formData.transactionId} | Bank: ${formData.bankName} | Date: ${formData.chequeDate}`;
        } else if (formData.paymentMode === 'NET_BANKING') {
            finalMaterialDetails = `TXN: ${formData.transactionId} | Bank: ${formData.bankName}`;
        }

        // Handle partner name for Anniversary / Wedding
        let finalOccasionName = formData.occasionName;
        if ((formData.purpose === 'Anniversary' || formData.purpose === 'Wedding') && formData.occasionPartnerName) {
            finalOccasionName = `${formData.occasionName} & ${formData.occasionPartnerName}`;
        }

        mutation.mutate({
            ...formData,
            amount: Number(formData.amount || 0),
            materialDetails: finalMaterialDetails,
            occasionName: finalOccasionName,
            mobile: formData.mobile ? `${formData.mobileCountryCode} ${formData.mobile}` : '',
            whatsappNumber: formData.whatsappNumber ? `${formData.whatsappCountryCode} ${formData.whatsappNumber}` : ''
        });
    };
        ...
        <div className="mx-auto max-w-4xl p-6">
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate(-1)} type="button" className="rounded-full p-2 hover:bg-slate-100">
                        <ArrowLeft size={20} className="text-slate-600" />
                    </button>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">New Donation Receipt</h1>
                        <p className="text-sm text-slate-500">Enter donor details and print bill book receipt</p>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <section className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 p-5 rounded-xl bg-slate-50 border border-slate-100">
                        <div>
                            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Preferred Prayer Date</label>
                            <input type="date" name="preferredPrayerDate" value={formData.preferredPrayerDate} onChange={handleChange} className="w-full rounded-lg border border-slate-200 p-2.5 outline-none focus:border-emerald-500 bg-white" />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Photograph (Optional)</label>
                            <input type="file" accept="image/*" onChange={handleImageUpload} className="w-full rounded-lg border border-slate-200 p-2 outline-none focus:border-emerald-500 bg-white text-sm" />
                            {formData.honouredPersonImage && <p className="text-xs text-emerald-600 mt-1 font-semibold">Image attached successfully</p>}
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Special Prayer / Message</label>
                            <textarea name="specialPrayerMessage" value={formData.specialPrayerMessage} onChange={handleChange} className="w-full rounded-lg border border-slate-200 p-2 outline-none focus:border-emerald-500 bg-white" rows={2} placeholder="Optional message..." />
                        </div>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            <label className="mb-1 block text-sm font-semibold text-slate-700">Permanent Address</label>
                            <textarea name="permanentAddress" value={formData.permanentAddress} onChange={handleChange} className="w-full rounded-xl border border-slate-200 p-2.5 outline-none focus:border-emerald-500" rows={2} />
                        </div>
                        <div className="md:col-span-2">
                            <label className="mb-1 block text-sm font-semibold text-slate-700">Donor Mail ID</label>
                            <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full rounded-xl border border-slate-200 p-2.5 outline-none focus:border-emerald-500" />
                        </div>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-8 p-6 bg-emerald-50 rounded-xl border border-emerald-200">
                        <label className="flex items-center gap-3 cursor-pointer font-bold text-emerald-900 text-lg">
                            <input type="checkbox" name="wishToVisitHome" checked={formData.wishToVisitHome} onChange={handleChange} className="h-6 w-6 text-emerald-600 focus:ring-emerald-500 rounded" />
                            Donor Wishes to Visit the Home
                        </label>
                        {formData.wishToVisitHome && (
                            <div className="mt-4 md:w-1/2">
                                <label className="mb-1 block text-sm font-semibold text-emerald-800">Preferred Visit Date</label>
                                <input type="date" name="preferredVisitDate" value={formData.preferredVisitDate} onChange={handleChange} className="w-full rounded-xl border border-emerald-300 p-2.5 outline-none focus:border-emerald-500 bg-white" />
                            </div>
                        )}
                    </div>
                </section>
                ...
                {mutation.isError && (
                    <div className="mb-4 rounded-xl bg-red-50 p-4 border border-red-200 text-red-700 font-semibold text-center">
                        Failed to save the donation. Please ensure your backend server is running and try again.
                    </div>
                )}
                ...
                <div className="flex justify-end pt-4 pb-12">
                    <button
                        ...
                        className="inline-flex h-14 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-10 font-bold text-white transition-colors hover:bg-emerald-800 disabled:opacity-50 text-lg shadow-lg"
                    >
                        <Save size={24} />
