...
export function RationProducts() {
    ...
    const rationProducts = useMemo(() => {
        ...
        return products
            .filter((product) => product.category.toLowerCase() === rationCategory)
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const product = await createProduct.mutateAsync({ name: productName, category: rationCategory })
    ...
    return (
        <div className="flex h-full flex-col">
            <PageHeader
                ...
                subtitle="Live ration product master and opening stock for inventory tracking."
                breadcrumbs={[{ label: 'UEC' }, { label: 'Inventory' }, { label: 'Ration Products' }]}
            />

            <ActionBar onAdd={() => setDrawerOpen(true)} addLabel="Add Ration Product" />

            <FilterSection
                searchQuery={searchQuery}
                onSearchChange={(event) => setSearchQuery(event.target.value)}
                searchPlaceholder="Search ration product..."
            />
            ...
            <DataTable
                ...
                emptyStateMessage="No live ration products found"
            />

            <Drawer
                isOpen={drawerOpen}
                onClose={() => setDrawerOpen(false)}
                title="Add Ration Product"
                size="md"
            >
