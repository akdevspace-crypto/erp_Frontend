...
export function Modal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal panel */}
            <div className="relative transform overflow-hidden rounded-[32px] border border-white/20 dark:border-white/5 bg-white dark:bg-black text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg z-10 p-2">
                ...
                <div className="bg-gray-50/50 dark:bg-black/20 border-x border-b border-gray-50 dark:border-white/10 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 rounded-b-[24px]">
                    {onConfirm && (
                        <button
                            type="button"
                            className={cn("inline-flex w-full justify-center rounded-xl border border-transparent px-5 py-2.5 text-[13.5px] font-bold shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto active:scale-95", btnClasses[type])}
                            onClick={onConfirm}
                        >
                            {confirmLabel}
                        </button>
